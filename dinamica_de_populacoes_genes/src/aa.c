...
#define NG 800 // numero de geragoes = 800 anos t=1,2anos
#define NF 250// numero de arquivos gerados

#define LS 0

#define pAm_die 0.03 // probabilidade de morte do macho de fenotipo dominante
#define pAf_die 0.01 // probabilidade de morte da femea de fenotipo dominante
#define pam_die 0.01 // probabilidade de morte do macho de fenotipo recessivo
#define paf_die 0.01 // probabilidade de morte da femea de fenotipo recessivo
#define genesel 0 // aa = 0 | Aa = 10 | AA = 11
#define ctolsel  0.5 // chance de um individuo com o fenótipo selecionado reproduzir com um sem 
#define ctolnsel 0.25 // chance de dois individuos sem o fenotipo selecionado interagirem entre si
...
void genes(struct INDV *in, int combinacao, const gsl_rng *w, int fio){
	if(in[fio].sexo == 1)
		in[fio].p_die = pAm_die;
	else
		in[fio].p_die = pAf_die;
	...
	if(in[fio].gene == 0 && in[fio].sexo == 1)
		in[fio].p_die = pam_die;
	else if(in[fio].gene == 0 && in[fio].sexo == 2)
		in[fio].p_die = paf_die;
}
			...
			if(in[n].ezst == 1){
				...
				for(j = 0; j < NI; j++){
					if(in[j].ezst == 1 && in[j].sexo != in[n].sexo){
						...
						if(dy>TR){dy-=TR;}
						if(dy<0){dy+=TR;}
						if(in[n].gene != genesel && in[j].gene != genesel){
						...
						}else if(in[n].gene != in[j].gene && (in[n].gene != genesel || in[j].gene != genesel) && gsl_rng_uniform(w) < ctolsel){
							if(sqrt(dx*dx+dy*dy) < R){
								for(f = 0; f < NI; f++){
									if(in[f].ezst == 0){
										in[f].ezst = 1;
						...
						}else if(gsl_rng_uniform(w) < ctolnsel){
							if(sqrt(dx*dx+dy*dy) < R){
								for(f = 0; f < NI; f++){
									if(in[f].ezst == 0){
										in[f].ezst = 1;
